...
```{r ci, eval=TRUE}
se <- sd(samp) / sqrt(60)
lower <- sample_mean + qnorm(p=.025)* se
upper <- sample_mean + qnorm(p=.975) * se
c(lower, upper)
```
...
-   Pick a confidence level of 80%. What is 
    the appropriate critical value? (use qnorm(p=.10) and qnorm(p=.90))
